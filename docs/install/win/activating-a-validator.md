...
It will take a significant while for the nodes in the network to process a deposit. Meanwhile, leave both Command Prompt windows open and running. Once the node is actived by the ETH2 network, the validator will immediately begin performing its responsibilities. The validator is now awaiting its first assignment from the network.
