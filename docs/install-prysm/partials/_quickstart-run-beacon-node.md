...
Syncing from a checkpoint usually takes a couple of minutes. See [Sync from a checkpoint](/configure-prysm/sync-from-checkpoint.md) for more information about this feature.
...
<p>Congratulations - you’re now running a <strong>full Ethereum node</strong>. To check the status of your node, visit <a href='/monitoring-alerts-metrics/check-node-and-validator-status'>Check node and validator status</a>.</p>
